...
#             self.lifetime -= 1
#         else:
#             print(len(pickle.dumps(['targetdied', [self]])))
#             client.sendto(pickle.dumps(['targetdied', [self]]), server)
#             ownbullets = [bullet for bullet in ownbullets if bullet.index != self.index]
#     def isHitted(self):
#         global ownbullets
#         global client
...
#                 else:
#                     player.xp += 2
#                 client.sendto(pickle.dumps(['targethitplayer', [p[NAME], p[HEALTH]]]), server)
#                 flashing = flash('white')
#                 flashing.start()
#     def botHitted(self):
#         global ownbullets
...
#                 else:
#                     player.xp += 1
#                 client.sendto(pickle.dumps(['targethitbot', [b[0], b[3]]]), server)
#                 flashing = flash('white')
#                 flashing.start()
...
#             elif ev in (ord('Q'), ord('q'), -113):
#                 print('abbrechen')
#                 client.sendto(pickle.dumps(['playerdata', [player.name, [player.x, player.y], player.direction, 0]]), server)
#                 runthreads = False
#                 sleep(0.1)
#                 client.close()
#                 quit()
...
#         while runthreads:
#             for b in ownbullets:
#                 client.sendto(pickle.dumps(['target', [b, b.direction, [b.x, b.y]]]), server)
#                 sleep(0.05)
#             client.sendto(pickle.dumps(['playerdata', [player.name, [player.x, player.y], player.direction, player.health]]), server)
#             sleep(0.2)
# class download(threading.Thread):
#     def __init__(self):
#         threading.Thread.__init__(self)
...
#         while runthreads:
#             try:
#                 someplayer, addr = client.recvfrom(2048)
#                 weg = False
#                 if pickle.loads(someplayer)[0] == 'playerdata':
#                     if pickle.loads(someplayer)[1][3] <= 0:
#                         for item in players:
